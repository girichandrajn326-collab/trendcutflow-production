  ...
  try {
    ...
    if (action === "transcribe") {
      ...
      try {
        ...
        if (!GROQ_KEY) {
          const msg = "GROQ_API_KEY secret is not configured — cannot transcribe. Add it in Supabase project settings under Edge Function Secrets.";
          await updateLog(supabase, logId, "error", msg, "GROQ_KEY_MISSING", 0);
    ...
    if (action === "detect-clips") {
      ...
      try {
        ...
        if (!OPENAI_KEY) {
          const msg = "OPENAI_API_KEY secret is not configured — cannot detect clips. Add it in Supabase project settings under Edge Function Secrets.";
          await updateLog(supabase, logId, "error", msg, "OPENAI_KEY_MISSING", 0);
          return json({ error: msg }, 500);
        }
